...
describe('visitsToCsv', () => {
  ...
  it('omits the share/post-tax columns entirely when partnerSplit is off', () => {
    const csv = visitsToCsv([row()], { filterDescription: 'All time', partnerSplit: false, ownShareLabel: 'BM' });
    const lines = csv.split('\n');
  ...
  it('produces just the description, header, and a zero totals line for an empty result', () => {
    const csv = visitsToCsv([], { filterDescription: 'No visits match', partnerSplit: false, ownShareLabel: 'BM' });
    const lines = csv.split('\n');
