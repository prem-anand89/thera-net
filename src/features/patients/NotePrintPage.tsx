...
/**
 * The heavy note's letterhead-through-footer content, with no toolbar/back
 * link of its own — extracted so InsurerPacketPage.tsx can compose it
 * alongside the session log and invoice sections in one print job, without
 * duplicating this ~300-line render. NotePrintPage wraps this with its own
 * print toolbar; this component only ever receives a heavy (non-session)
 * note — callers are responsible for that check (NotePrintPage's own
 * session guard above; InsurerPacketPage only ever fetches completed
      ...
      {payload?.referral && (payload.referral.referringPhysician || payload.referral.diagnosis) && (
        <Section title="Referral & Diagnosis">
          ...
          <Row
            label="Referral date"
            value={
              payload.referral.referralDate ? formatDateDMY(payload.referral.referralDate) : null
            }
          />
  ...
  if (note.noteMode === 'session') {
    return (
      <div className="p-8 text-sm text-[var(--muted)]">
        Session notes print from the multi-visit session log, not individually.{' '}
        <Link
