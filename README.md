...
- **Today-first workspace** — default landing page showing today's visits with payment state at a glance (Paid / Collect ₹X / Package / Free — the last two both mean ₹0 billed, but for different reasons: a package session already billed elsewhere vs. a standalone complimentary visit) as boxed cards on phone, a table on tablet/desktop, and a Packages panel (Open/Stale/All, "Mine only" for anyone with a linked therapist record) for tracking sessions still owed.
...
- **Today section** — visits entered today with payment state chips (Paid / Collect ₹X / Package / Free), organized as table rows
